...
describe Hydra::Derivatives::Video::Processor do
  ...
  describe ".config" do
    ...
    it "should be configurable" do
      expect(subject).to receive(:encode_file).with("mp4", { Hydra::Derivatives::Ffmpeg::OUTPUT_OPTIONS => "-s 320x240 -vcodec mpeg4 -acodec aac -strict -2 -g 30 -b:v 345k -ac 2 -ab 96k -ar 44100", Hydra::Derivatives::Ffmpeg::INPUT_OPTIONS => "" })
      subject.process
    end
  end
  ...
  context "when arguments are passed as a hash" do
    context "and a video format is requested" do
      ...
      it "creates a fedora resource and infers the name" do
        expect(subject).to receive(:encode_file).with("webm", { Hydra::Derivatives::Ffmpeg::OUTPUT_OPTIONS => "-s 320x240 -vcodec libvpx -acodec libvorbis -g 30 -b:v 345k -ac 2 -ab 96k -ar 44100", Hydra::Derivatives::Ffmpeg::INPUT_OPTIONS => "" })
        subject.process
      end
    end
